...
bool RECEIVE_ATTR RCSwitch::testNexaProtocol(unsigned int changeCount) {
    uint64_t code = 0;
    uint8_t tmp = 0;
    const unsigned int delay = RCSwitch::timings[0] / 380; // What ever....
    ...
    if (diff(RCSwitch::timings[0], 10700) < 200
        &&
        diff(RCSwitch::timings[1],   380) <  80
        &&
        diff(RCSwitch::timings[2],  2700) < 100)
    {
        // Found preamble
        for (unsigned int i = 3; i < changeCount - 2; i += 2)
        {
            if (tmp > 2)
            {
                //Serial.printf("Ehhh, tmp too large, something wrong... %d\n", tmp);
                return false;
            }
            else if (diff(RCSwitch::timings[i], 325) < 40 && diff(RCSwitch::timings[i+1],  245) < 40) // max 350 min 292 avg 324.66 || max  277 min  218 avg  246.70
            {
                tmp++;
            }
            else if (diff(RCSwitch::timings[i], 325) < 40 && diff(RCSwitch::timings[i+1], 1315) < 50) //                            || max 1352 min 1286 avg 1315.57
            {
                tmp++;
                //printf(" %d", tmp);
                code <<= 2;
            ...
            {
                //Serial.printf("Doesn't recognize this pattern (%d, %d), giving up\n",
    ...
    {
        //Serial.printf("Did not find preamble\n");
        return false;
    }

    RCSwitch::nReceivedValue = code;
    RCSwitch::nReceivedBitlength = (changeCount - 3) / 2;
    RCSwitch::nReceivedDelay = delay;
    RCSwitch::nReceivedProtocol = numProto + 1; // What ever....!
    return true;
}
...
void RECEIVE_ATTR RCSwitch::handleInterrupt() {
  ...
  if (duration > RCSwitch::nSeparationLimit) {
    ...
    if ((repeatCount==0) || (diff(duration, RCSwitch::timings[0]) < 200)) {
      ...
      repeatCount++;
      if (repeatCount == 2) {
        if (! testNexaProtocol(changeCount))
        {
